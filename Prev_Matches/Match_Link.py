def match_link():
    ...
    import os
    from Prev_Matches import Season
    ...
    write_links_to_file({year: [link for link in year_links if 'full-scorecard' in link] for year, year_links in all_links.items()}, 'Match_Links')
    write_links_to_file({year: [link for link in year_links if 'match-impact-player' in link] for year, year_links in all_links.items()}, 'MVP_Links')
    write_links_to_file({year: [link for link in year_links if 'match-statistics' in link] for year, year_links in all_links.items()}, 'Stats_Links')
    write_links_to_file({year: [link for link in year_links if 'match-overs-comparison' in link] for year, year_links in all_links.items()}, 'Overs_Comparison_Links')
    write_links_to_file({year: [link for link in year_links if 'points-table-standings' in link] for year, year_links in all_links.items()}, 'Points_Table_Links')
