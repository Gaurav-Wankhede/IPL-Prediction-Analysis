...
for i, table in enumerate(bowling_tables):
    ...
    if 'BOWLING' in table.columns:
        ...
        # Query database to get the last used Bowling_ID
        last_id_query = f"SELECT MAX(Bowling_ID) FROM {table_name}"
        last_id_df = pd.read_sql_query(last_id_query, conn)
        last_id = last_id_df.iloc[0, 0] if not last_id_df.empty else 0

        # Add primary key column based on continuation of last used Bowling_ID
        table['Bowling_ID'] = range(last_id + 1, last_id + 1 + len(table))
...
combine_table.to_csv('../LatestBowling_table.csv', index=False, encoding='utf-8-sig')
